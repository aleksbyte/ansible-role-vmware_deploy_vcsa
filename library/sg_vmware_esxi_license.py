        ...
        if key is not None:
            ...
            # assign to ESXi server
                ...
                if 'esx' not in key.editionKey:
                    module.debug('License key "%s" edition "%s" is not suitable for ESXi server' % (license, key.editionKey))
            else:
                module.debug('Parameters "cluster_name" or "esxi_hostname" have not been provided so we are not assigning this licence')

            # if we have found on which entity to add the licence then we can try to assign it
            ...
            if entityId:
                ...
                if not assigned_license or (len(assigned_license) != 0 and assigned_license[0].assignedLicense.licenseKey != license):
                    ...
                    result['changed'] = True
            # always return the list of keys
            result['licenses'] = pyv.list_keys(lm.licenses)
        else:
